...
public class ServiceQueryAdapter extends  RecyclerView.Adapter<ServiceQueryAdapter.MyViewHolder>{       // Apparently this is not a normal Activity as most of the java classes in the project,
                                                                                                        // this java class works as the backend for the RecyclerView that is used in a QueryActivity,
                                                                                                        // it allows the RecyclerView to show the contents of serviceArrayListQuery as well as provides the backend for the pieces that RecyclerView displays,
                                                                                                        // e.g. the contents of serviceArrayListQuery are displayed and this Adapter allows a user to click on such displayed client and execute equivalent code after a click
    ...
    ArrayList<Service> serviceArrayListQuery;   // Initializing the object serviceArrayListQuery (ArrayList<Service>), it is an array of objects containing all the services that came as a result of query
                                                // in other words it holds all the services that have went through the query filters specified by a user
    ...
    public ServiceQueryAdapter(Context context, ArrayList<Service> serviceArrayList) {  // In this place we are creating a constructor of a ServiceQueryAdapter
        this.context = context;
        this.serviceArrayListQuery = serviceArrayList;
    }

    @NonNull
    @Override
    public ServiceQueryAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {   // This function is called, because of the fact that RecyclerView needs a new ViewHolder in order to represent an item,
        ...
        return new ServiceQueryAdapter.MyViewHolder(v); // This function returns a new MyViewHolder with a parameter v (which was created in this function)
    }

    @Override
    public void onBindViewHolder(@NonNull ServiceQueryAdapter.MyViewHolder holder, int position) {  // This method is called by the RecyclerView in order to display the data at the specified position,
        ...
        Service service = serviceArrayListQuery.get(position);  // This line of code is fetching the service from a specified position in the array and saving it into a service (Service) object
        ...
        holder.itemView.setOnClickListener(new View.OnClickListener() { // In this place we are setting the OnClickListener that is required to perform equivalent code
            ...
            public void onClick(View view) {    // The code of this method is performed once the user has clicked on one of the items displayed
                ...
                intent.putExtra("uComingFrom","QueryActivity"); // With an Intent we can pass variables, so we are passing the "uComingFrom", in order to later access it in the next activity
    ...
    @Override
    public int getItemCount() { // This function returns the number of services that have came through the query filters specified by a user
        return serviceArrayListQuery.size();    // We are returning the size of the serviceArrayListQuery which means that we are returning the number of services that were found as a result of query,
                                                // in other words it returns the amount of services in the database that have came through the filters specified by a user
    }
