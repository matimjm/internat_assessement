...
public class NewDeviceActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener, AdapterView.OnItemSelectedListener {
            ...
            btnNewDeviceAdd.setOnClickListener(new View.OnClickListener() { // We are setting the OnClickListener to the btnNewDeviceAdd, in order to constantly listen when the button was click,
                ...
                public void onClick(View view) {    // The code under this method is executed once the btnNewDeviceAdd button is clicked
                    ...
                    if (!IMEIOrSNum_txt.isEmpty() || !modelName.equals("none")) {
                        if (validationRulesIMEIorSNUM(IMEIOrSNum_txt)){
                            ...
                            Toast.makeText(NewDeviceActivity.this, "IME or Serial Number is ivalid", Toast.LENGTH_SHORT).show();
                        }
    ...
    public static boolean validationRulesIMEIorSNUM(String ImeiOrSNum){ // this function checks whether the inputted  by the user IMEI or serial number has a valid format
        // validations here in this regex formula:
        // 1. it can contain only a-z (ignore case) and 0-9 characters
        // 2. it cannot contain any whitespaces
        // 3. has to be between 1 and 50 characters
        String regex = "^[a-zA-Z0-9]{1,50}$";
        System.out.println(ImeiOrSNum.matches(regex) + "IMEI\n");
        return ImeiOrSNum.matches(regex);
    }
